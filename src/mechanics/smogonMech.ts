import { AbilityName, StatID, Terrain } from "@smogon/calc/dist/data/interface";
import {
  getBerryResistType,
  getFlingPower,
  getItemBoostType,
...
import { Generation, toID } from "@pkmn/data";
...
export function calculateBasePowerSMSSSV(
  ...
  switch (move.name) {
    ...
    case "Wring Out":
      basePower =
        100 * Math.floor((defender.curHP() * 0x1000) / defender.maxHP());
      basePower =
        Math.floor(Math.floor((120 * basePower + 0x800 - 1) / 0x1000) / 100) ||
        1;
      desc.moveBP = basePower;
      break;
    case "Hard Press":
      basePower =
        100 * Math.floor((defender.curHP() * 0x1000) / defender.maxHP());
      basePower =
        Math.floor(Math.floor((100 * basePower + 0x800 - 1) / 0x1000) / 100) ||
        1;
      desc.moveBP = basePower;
      break;
    case "Tera Blast":
  ...
  basePower = OF16(
    Math.max(
      1,
      roundDown((basePower * chainMods(bpMods, 41, 2097152)) / 0x1000)
    )
  );
  if (
    attacker.teraType &&
...
export function calculateBPModsSMSSSV(
  ...
  let resistedKnockOffDamage =
    ...
    (defender.name.includes("Ogerpon-Cornerstone") &&
      defender.hasItem("Cornerstone Mask")) ||
    (defender.name.includes("Ogerpon-Hearthflame") &&
      defender.hasItem("Hearthflame Mask")) ||
    (defender.name.includes("Ogerpon-Wellspring") &&
      defender.hasItem("Wellspring Mask")) ||
  ...
  if (
    ...
    if (type1Effectiveness * type2Effectiveness >= 2) {
      bpMods.push(0x1555);
      desc.moveBP = basePower * (0x1555 / 0x1000);
    }
  }
  ...
  if (attacker.hasAbility("Supreme Overlord") && attacker.alliesFainted) {
    const powMod = [0x1000, 0x119a, 0x1333, 0x14cd, 0x1666, 0x1800];
...
export function calculateAttackSMSSSV(
  ...
  attack = OF16(
    Math.max(1, roundDown((attack * chainMods(atMods, 410, 131072)) / 0x1000))
  );
  return attack;
}
...
export function calculateAtModsSMSSSV(
  ...
  if (
    ...
    atMods.push(gen.num >= 9 ? 0x14cd : 0x1800);
    desc.attackerAbility = attacker.ability;
...
export function calculateDefenseSMSSSV(
  ...
  return OF16(
    Math.max(1, roundDown((defense * chainMods(dfMods, 410, 131072)) / 0x1000))
  );
}
...
export function calculateBaseDamageSMSSSV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  basePower: number,
  ...
  const isSpread =
    ...
    ["allAdjacent", "allAdjacentFoes"].includes(move.target);
  if (isSpread) {
    baseDamage = roundDown(OF32(baseDamage * 0xc00) / 0x1000);
  }

  if (attacker.hasAbility("Parental Bond (Child)")) {
    baseDamage = roundDown(OF32(baseDamage * 1024) / 0x1000);
  }
  ...
  if (
    ...
    baseDamage = roundDown(OF32(baseDamage * 0x1800) / 0x1000);
    desc.weather = field.weather;
    ...
    if (
      ...
      baseDamage = roundDown(OF32(baseDamage * 0x800) / 0x1000);
      desc.weather = field.weather;
    }
  }
...
export function calculateFinalModsSMSSSV(
  ...
  if (
    ...
    finalMods.push(field.gameType !== "Singles" ? 2732 : 0x800);
    desc.isReflect = true;
  } else if (
  ...
  if (attacker.hasItem("Expert Belt") && typeEffectiveness > 1 && !move.isZ) {
    ...
    if (timesUsedWithMetronome <= 4) {
      finalMods.push(0x1000 + timesUsedWithMetronome * 819);
    } else {
      finalMods.push(0x2000);
    }
    desc.attackerItem = attacker.item;
