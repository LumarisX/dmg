...
describe('encode', () => {
  test('encodeURL', () => {
    const raw =
      '{Gen 4} +1 252 SpA Gengar @ Choice Specs [Focus Blast] vs. ' +
      '0 HP / 172+ SpD 74% Blissey weather:Sand';
    const encoded =
      '(Gen_4)_+1_252_SpA_Gengar_*_Choice_Specs_(Focus_Blast)_vs._' +
  ...
  test('encode', () => {
    ...
    expect(
      enc('(Gen 7 Doubles) Ultra Necrozma @ Ultranecrozium Z [Light That Burns The Sky] vs. Kyogre')
    ).toEqual(
  ...
  test('getNature', () => {
    expect(() => getNature({plus: 'hp', minus: 'atk'}, undefined)).toThrow(
      'Natures cannot modify HP'
    );
    expect(getNature({plus: 'atk', minus: 'spa'}, undefined)).toBe('Adamant');
    expect(getNature({}, {})).toBeUndefined();
    expect(
      getNature({plus: 'atk'}, {hp: 0, atk: 0, def: 0, spa: 0, spd: 0, spe: 0})
    ).toBeUndefined();
