...
import {Data, GenerationNum, Generations, Weather} from '@pkmn/data';
...
const NATDEX_EXISTS = (d: Data) => {
  ...
  if ('isNonstandard' in d && d.isNonstandard) {
    ...
    if (['CAP', 'Custom', 'Future'].includes(d.isNonstandard)) {
      return false;
    }
...
const gens = new Generations(Dex as any, NATDEX_EXISTS);
...
// describe('calc', () => {
//   describe('Multi-Gen', () => {
//     inGens(gens, 4, 7, ({gen, calculate, Pokemon, Move}) => {
//       test(`Grass Knot (gen ${gen})`, () => {
//         const result = calculate(gen)(Pokemon('Groudon'), Pokemon('Groudon'), Move('Grass Knot'));
...
//         const result = calculate(gen)(Pokemon('Arceus', {item: 'Meadow Plate'}), Pokemon('Blastoise'), Move('Judgment'));
...
//     inGens(gens, 1, 9, ({gen, calculate, Pokemon, Move}) => {
...
//           const result = calculate(gen)(mew, vulpix, move);
//           expect(result.damage.mode).toBe(50);
//           /* expect(result.toString()).toBe(
//             gen.num < 3
...
//         expect(calculate(gen)(Pokemon('Snorlax'), Pokemon('Gengar'), Move('Hyper Beam')).damage).toBe(0);
//       });
//     });

//     inGens(gens, 1, 9, ({gen, calculate, Pokemon, Move}) => {
//       test(`Non-damaging (gen ${gen})`, () => {
//         const result = calculate(gen)(Pokemon('Snorlax'), Pokemon('Vulpix'), Move('Barrier'));
//         expect(result.damage.mode).toBe(0);
//         /* expect(result.toString()).toBe(
...
//         expect(calculate(gen)(Pokemon('Snorlax'), Pokemon('Chansey', {volatiles: ['protect']}), Move('Hyper Beam')).damage).toBe(0);
//       });
//     });

//     inGens(gens, 1, 9, ({gen, calculate, Pokemon, Side, Move}) => {
//       test(`Critical hits ignore attack decreases (gen ${gen})`, () => {
//         const mew = Pokemon('Mew', {status: 'brn'});
//         const vulpix = Pokemon('Vulpix');
//         const explosion = Move('Explosion', {crit: true});
//         let result = calculate(gen)(mew, Side(vulpix, {sideConditions: ['Reflect']}), explosion);
//         mew.boosts.atk = 2;
//         vulpix.boosts.def = 2;
//         if (gen.num < 2) {
...
//           );*/
//         }
//         explosion.crit = false;
//         result = calculate(gen)(mew, Side(vulpix, {sideConditions: ['Reflect']}), explosion);
//         if (gen.num === 1) {
//           expect(result.range).toEqual([102, 120]);
//         } else if (gen.num === 2) {
//           expect(result.range).toEqual([149, 176]);
//         } else if (gen.num > 2 && gen.num < 5) {
...
//         const result = calculate(gen)(Pokemon('Mew'), Pokemon('Gengar'), Move('Struggle'));
//         if (gen.num < 2) {
...
//     inGens(gens, 3, 9, ({gen, calculate, Pokemon, Move, Field}) => {
...
//         for (const {weather, damage} of weathers) {
//           const dmg = gen.num === 3 ? damage.adv : gen.num === 4 ? damage.dpp : damage.modern;
//           // const [atk, def] = gen.num === 3 && type === 'Rock' ? ['Atk', 'Def'] : ['SpA', 'SpD'];
//           const result = calculate(gen)(Pokemon('Castform'), Pokemon('Bulbasaur'), Move('Weather Ball'), Field({weather: weather as Weather}));
...
//     inGens(gens, 6, 9, ({gen, calculate, Pokemon, Move}) => {
//       test(`Thousand Arrows and Ring Target Should negate damage nullfiers (gen ${gen})`, () => {
//         const result = calculate(gen)(Pokemon('Zygarde'), Pokemon('Swellow'), Move('Thousand Arrows'));
...
//           // let result = calculate(gen)(Pokemon('Mew', {ivs}), Pokemon('Mew', {evs}), Move('Psychic'));
//           /* expect(result.toString()).toBe(
//             "0 SpA 9 IVs Mew Psychic vs. 9 HP / 9 SpD Mew: 43-51 (12.5 - 14.8%) -- possible 7HKO"
//           );*/
//           // result = calculate(gen)(Pokemon('Mew', {evs}), Pokemon('Mew', {ivs}), Move('Psychic'));
//           /* expect(result.toString()).toBe(
...
//     inGens(gens, 4, 9, ({gen, calculate, Pokemon, Move}) => {
//       const zapdos = Pokemon('Zapdos', {item: 'Iron Ball'});
//       if (gen.num === 4) {
//         test(`Iron Ball negates ground immunities (gen ${gen})`, () => {
//           const result = calculate(gen)(Pokemon('Vibrava'), zapdos, Move('Earthquake'));
...
//         const result = calculate(gen)(Pokemon('Poliwrath'), Pokemon('Mismagius', {item: 'Iron Ball'}), Move('Mud Shot'));
...
//     inGens(gens, 5, 9, ({gen, calculate, Pokemon, Move, Side}) => {
...
//           const result = calculate(gen)(Pokemon('Abomasnow'), Side(dragonite2, {sideConditions: ['Stealth Rock']}), Move('Blizzard'));
...
//         const result = calculate(gen)(Pokemon('Abomasnow'), dragonite1, Move('Ice Shard'));
...
//         const result = calculate(gen)(Pokemon('Abomasnow'), dragonite, Move('Ice Shard'));
...
//   inGen(gens.get(8), ({gen, Pokemon}) => {
//     test(`Pokemon should double their HP stat when dynamaxing (gen ${gen})`, () => {
//       const munchlax = Pokemon('Munchlax', {volatiles: ['dynamax']});
//       expect(munchlax.hp).toBe(822);
//     });
//   });

//   inGens(gens, 7, 9, ({gen, calculate, Pokemon, Move, Field}) => {
...
//       let result = calculate(gen)(Mewtwo, Milotic, Psystrike, field);
//       if (gen.num < 8) {
...
//         );*/
//       }
//       result = calculate(gen)(Mewtwo, Milotic, sPunch, field);
...
//       if (gen.num === 6) {
//         expect(result.damage.mode).toEqual([
...
//       result = calculate(gen)(Pokemon('Kangaskhan-Mega', {level: 88}), Pokemon('Amoonguss'), Move('Seismic Toss'));
//       expect(result.damage.mode).toEqual([88, 88]);
//       /* expect(result.toString()).toBe(
//         "Lvl 88 Parental Bond Kangaskhan-Mega Seismic Toss vs. 0 HP Amoonguss: 176-176 (47.6 - 47.6%) -- guaranteed 3HKO"
//       );*/

//       result = calculate(gen)(Pokemon('Kangaskhan-Mega', {evs: {atk: 252}}), Pokemon('Aggron', {level: 72}), Move('Power-Up Punch'));
//       if (gen.num === 6) {
//         /* expect(result.toString()).toBe(
...
//       result = calculate(gen)(Pokemon('Kangaskhan-Mega', {evs: {atk: 252}}), Pokemon('Lunala'), Move('Crunch'));
...
//   inGens(gens, 5, 9, ({gen, calculate, Pokemon, Move}) => {
...
//   inGens(gens, 7, 9, ({gen, calculate, Pokemon, Move}) => {
...
//   inGens(gens, 3, 9, ({gen, calculate, Pokemon, Move}) => {
...
//       const result = calculate(gen)(chansey, mew, seismicToss);
//       expect(result.damage.mode).toBe(25);
//       /* expect(result.toString()).toBe(
...
//       const seismicToss = Move('Seismic Toss');
//       const result = calculate(gen)(chansey, mew, seismicToss);
//       expect(result.damage.mode).toBe(55);
//       /* expect(result.toString()).toBe(
...
//         level: 30,
//         status: 'tox',
//         statusState: {toxicTurns: 1},
//         ivs: {hp: 0},
//       });
//       const seismicToss = Move('Seismic Toss');
//       const result = calculate(gen)(chansey, mew, seismicToss);
//       expect(result.damage.mode).toBe(1);
//       /* expect(result.toString()).toBe(
//         "Lvl 1 Chansey Seismic Toss vs. Lvl 30 0 HP 0 IVs Mew: 1-1 (1 - 1%) -- guaranteed 6HKO after toxic damage"
//       );*/
//     });
//   });
//   inGens(gens, 3, 9, ({gen, calculate, Pokemon, Move, Side, Field}) => {
...
//       );
//       const seismicToss = Move('Seismic Toss');
//       const result = calculate(gen)(chansey, mew, seismicToss, field);
//       expect(result.damage.mode).toBe(1);
//       /* expect(result.toString()).toBe(
...
//       const seismicToss = Move('Seismic Toss');
//       const result = calculate(gen)(chansey, mew, seismicToss, field);
//       expect(result.damage.mode).toBe(90);
//       /* expect(result.toString()).toBe(
...
//   inGen(gen, ({ calculate, Pokemon, Move, Side }) => {
...
//       const result = calculate(gen)(
//         Pokemon("Gengar"),
//         Side(Pokemon("Vulpix"), { sideConditions: ["Light SCreen"] }),
//         Move("Surf")
//       );
//       expect(result.toString()).toBe(
...
//   inGen(gen, ({ calculate, Pokemon, Move, Side, Field }) => {
//     test("Basic: Gengar vs. Chansey", () => {
//       const result = calculate(gen)(
//         Pokemon("Gengar"),
...
//         status: "brn",
//       });
//       const defender = Side(Pokemon("Umbreon", { boosts: { def: 6 } }), {
//         sideConditions: ["Reflect"],
//       });
...
//       const result = calculate(gen)(aerodactyl, zapdos, move);
...
//   inGen(gen, ({ calculate, Pokemon, Move, Field }) => {
//     test("Basic: Gengar vs. Chansey", () => {
//       const result = calculate(gen)(
//         Pokemon("Gengar", {
//           nature: "Mild",
...
//       let result = calculate(gen)(blastoise, cacturne, surf);
//       expect(result.range).toEqual([88, 104]);
//       expect(result.toString()).toBe(
//         "252 SpA Blastoise Surf vs. 0 HP / 0 SpD Cacturne: 88-104 (31.3 - 37%) -- 76.6% chance to 3HKO"
//       );

//       cacturne.ability = "waterabsorb" as ID;
//       result = calculate(gen)(blastoise, cacturne, surf);
//       expect(result.damage.mode).toBe(0);
//     });
...
//   inGen(gen, ({ calculate, Pokemon, Move }) => {
//     test("Basic: Gengar vs. Chansey", () => {
//       const result = calculate(gen)(
//         Pokemon("Gengar", {
//           item: "Choice Specs",
...
//       let result = calculate(gen)(pinsir, gengar, earthquake);
//       expect(result.damage.mode).toBe(0);

//       pinsir.ability = "moldbreaker" as ID;
//       result = calculate(gen)(pinsir, gengar, earthquake);
...
//       let result = calculate(gen)(ambipom, blissey, Move("Low Kick"));
...
//       result = calculate(gen)(ambipom, aggron, Move("Low Kick"));
...
//       const zMove = Move("Wood Hammer", { useZ: true, crit: true });
//       const result = calculate(gen)(abomasnow, hoopa, zMove);
...
//       let result = calculate(gen)(abomasnow, hoopa, Move("Wood Hammer"));
...
//       expect(recoil).toEqual([24, 28.3]);
//       // expect(recoil.text).toBe("24 - 28.3% recoil damage");

//       result = calculate(gen)(hoopa, abomasnow, Move("Drain Punch"));
...
//       expect(recovery).toEqual([161, 161]);
//       // expect(recovery.text).toBe("52.1 - 52.1% recovered");
//     });
//     test("Big Root", () => {
//       const bigRoot = Pokemon("Blissey", { item: "Big Root" });
//       const result = calculate(gen)(bigRoot, abomasnow, Move("Drain Punch"));
...
//       });
//       const result = calculate(gen)(
//         Side(abomasnow, { sideConditions: ["Helping Hand", "Tailwind"] }),
//         Side(hoopa, {
//           sideConditions: [
//             "Stealth Rocks",
//             "Spikes1",
...
//         hp: 184,
//       });
//       const result = calculate(gen)(smeargle, blissey, Move("Wring Out"));
...
//       const result = calculate(gen)(
//         Side(Pokemon("Mewtwo-Mega-Y", { evs: { spa: 196 } }), {
//           sideConditions: ["Helping Hand"],
//         }),
...
//         item: "Choice Specs",
//         hp: 340, // we need 149 base power Water Spout
...
//       expect(
//         calculate(gen)(
//           Side(kyogre, { sideConditions: ["Helping Hand"] }),
//           wynaut,
//           waterSpout,
//           field
//         ).range
...
//       const result = calculate(gen)(sawk, silvally, knockoff);
//       expect(result.toString()).toBe(
...
//       const result = calculate(gen)(sylveon, silvally, hypervoice);
//       expect(result.toString()).toBe(
...
//       const result = calculate(gen)(abomasnow, deerling, blizzard, hail);
//       expect(result.toString()).toBe(
...
//       const result = calculate(gen)(kyurem, jirachi, earthpower);
//       expect(result.toString()).toBe(
...
describe('Gen 9', () => {
  ...
  inGen(gen, ({calculate, Pokemon, Move, Side, Field}) => {
    ...
    //   let result = calculate(gen)(kingambit, aggron, Move("Iron Head"));
    ...
    //   result = calculate(gen)(kingambit, aggron, Move("Iron Head"));
    ...
    test('Electro Drift/Collision Course boost on Super Effective hits', () => {
      ...
      const calc = (move = Move('Electro Drift')) => calculate(gen)(attacker, defender, move).range;
    ...
    // function testQP(
    //   ability: string,
    //   field?: { weather?: Weather; terrain?: TerrainName }
    // ) {
    ...
    //   result = calculate(gen)(attacker, defender, Move("Revelation Dance"));
    ...
    test("Flower Gift, Power Spot, Battery, and switching boosts shouldn't have double spaces", () => {
      ...
      const defender = Pokemon('Vulpix', {switching: 'out'});
      const field = Field({
        weather: 'Sun',
      });
      const result = calculate(gen)(Side(attacker, {sideConditions: ['Flower Gift', 'Power Spot']}), defender, Move('Pursuit'), field);
    ...
    describe('Descriptions', () => {
      inGen(gen, ({gen, calculate, Pokemon, Move}) => {
        test('displayed chances should not round to 100%', () => {
          ...
          expect(result.knockout().chance).toBeGreaterThanOrEqual(0.9995);
          // expect(result.text()).toBe('99.9% chance to 3HKO');
        });
