...
describe("Context", () => {
  test("Field", () => {
    ...
    expect(context.field.weather?.onModifyAtk?.(p1)).toBe(1);
    expect(relevancy.field.weather).toBe(true);

    expect(context.field.terrain?.onModifyAtk?.(p1)).toBeUndefined();
    expect(relevancy.field.terrain).toBeUndefined();
    expect(context.field.terrain?.onModifyDef?.(p1)).toBe(2);
    expect(relevancy.field.terrain).toBe(true);

    expect(
      context.field.pseudoWeather["gravity"]?.onModifyAtk?.(p1)
    ).toBeUndefined();
    expect(relevancy.field.pseudoWeather["gravity"]).toBeUndefined();

    expect(context.field.pseudoWeather["gravity"]?.onModifySpA?.(p1)).toBe(3);
