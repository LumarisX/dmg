...
export interface PokemonOptions extends Partial<Omit<LumState.Pokemon, OverriddenFields>> {
  name?: string;
  weightkg?: number;
  item?: string;
  ability?: string;
  ...
  status?: string;
  hpPercent?: number;
  volatiles?: string[] | LumState.Pokemon['volatiles'];
...
export interface FieldOptions {
  weather?: string;
  terrain?: string;
  pseudoWeather?: string[] | LumState.Field['pseudoWeather'];
}

export interface SideOptions {
  sideConditions?: string[] | LumState.Side['sideConditions'];
  abilities?: string[];
  atks?: number[];
}

export interface PositionOptions {
  positionConditions?: string[] | LumState.Side['sideConditions'];
}
 ...
 */
export class LumState {
  readonly gameType: GameType;
  readonly gen: Generation;
  readonly p1: LumState.Position;
  readonly p2: LumState.Position;
  readonly move: LumState.Move;
  readonly field: LumState.Field;

  constructor(
    gen: Generation,
    attacker: LumState.Position | LumState.Pokemon,
    defender: LumState.Position | LumState.Pokemon,
    move: LumState.Move,
    field: LumState.Field = {pseudoWeather: {}},
    gameType: GameType = 'singles'
  ) {
    this.gameType = gameType;
    this.gen = gen;
    this.p1 = 'species' in attacker ? {pokemon: attacker} : attacker;
    this.p2 = 'species' in defender ? {pokemon: defender} : defender;
    this.move = move;
    this.field = field;
  }

  /** Serializes State, collapsing immutable data structures that have circular references. */
  static toJSON(s: LumState) {
    const move = {} as any;
    const base = s.gen.moves.get(s.move.name);
    for (const key in s.move) {
      if (Object.prototype.hasOwnProperty.call(s.move, key)) {
        const val = s.move[key as keyof LumState.Move];
  ...
  static createField(gen: Generation, options: FieldOptions = {}) {
    const field: Partial<LumState.Field> = {};
    ...
    return field as LumState.Field;
  }

  /** Factory method helper for creating `State.Side`. */
  static createSide(gen: Generation, pokemon: LumState.Pokemon[], options: SideOptions = {}): LumState.Side {
    return {
      sideConditions: setConditions(gen, 'Side Condition', options.sideConditions),
      positions: pokemon.map(p => this.createPosition(gen, p)),
  ...
  static createPosition(gen: Generation, pokemon: LumState.Pokemon, options: PositionOptions = {}): LumState.Position {
    return {
      pokemon: pokemon,
    };
  }
  ...
  static createPokemon(gen: Generation, name: string, options: PokemonOptions = {}, move: string | {name?: string} = '') {
    const pokemon: Partial<LumState.Pokemon> = {};
    ...
    return validateStats(gen, pokemon as LumState.Pokemon);
  }
  ...
  static createMove(
    ...
    const move: Partial<LumState.Move> = {hits: 1};
    ...
    return move as LumState.Move;
  }

  /** Mutates `pokemon` by merging in the details from the set from `sets` which best matches. */
  static mergeSet(gen: Generation, pokemon: LumState.Pokemon, move: string | DeepPartial<PokemonSet>, ...sets: DeepPartial<PokemonSet>[]) {
...
function bestMatch(pokemon: LumState.Pokemon, move: string | DeepPartial<PokemonSet>, ...sets: DeepPartial<PokemonSet>[]) {
...
function setItem(gen: Generation, pokemon: Partial<LumState.Pokemon>, name?: string) {
  if (name) {
...
function setNature(gen: Generation, pokemon: Partial<LumState.Pokemon>, name?: string, die = true) {
  if (name) {
...
function setValues(gen: Generation, pokemon: Partial<LumState.Pokemon>, type: 'evs' | 'ivs', vals?: Partial<StatsTable & {spc: number}>) {
...
function validateStats(gen: Generation, pokemon: LumState.Pokemon) {
