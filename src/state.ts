...
export class State {
  ...
  constructor(gen: Generation, sides: State.Side[], action: Action, field: State.Field = {pseudoWeather: {}}, gameType: GameType = 'singles') {
  ...
  static oneOnOne(
    ...
    return new State(gen, [asSide(attacker), asSide(defender)], {actor: SLOT_ATTACKER, target: SLOT_TARGET, move}, field, gameType);
  }
  ...
  withMove(move: State.Move): State {
    return move === this.action.move ? this : this.derive(this.sides, {...this.action, move});
  }
...
export namespace State {
  ...
  export interface Move extends DMove {
    ...
    consecutive?: number;
    useZ?: boolean;
    flags: {[flag: string]: 1 | 0 | boolean | undefined};
  }
}
