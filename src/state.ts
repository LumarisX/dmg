...
export interface PokemonOptions extends Partial<
  Omit<State.Pokemon, OverriddenFields>
> {
  name?: string;
  weightkg?: number;
  item?: string;
...
export class State {
  ...
  static createField(gen: Generation, options: FieldOptions = {}) {
    ...
    field.pseudoWeather = setConditions(
      gen,
      "Pseudo Weather",
  ...
  static createPokemon(
    ...
    if (options.species && options.species !== species) {
      throw new Error(
        `Species mismatch: ${options.species.name} does not match ${species.name}`,
      );
    }
    ...
    if (pokemon.weighthg < 1)
      throw new Error(`weighthg of ${pokemon.weighthg} must be at least 1`);
    ...
    setAbility(
      gen,
      pokemon as { species: Specie; ability?: ID },
      options.ability,
    );
    ...
    if (options.status) {
      ...
      if (kind !== "Status") {
        throw new Error(
          `'${status} is a ${kind} not a Status in generation ${gen.num}`,
        );
      }
    ...
    if (options.statusState) {
      if (options.statusState.toxicTurns) {
        ...
        if (pokemon.status !== "tox") {
          throw new Error(
            `toxicTurns set to ${turns} but the Pokemon's status is not 'tox'`,
          );
        }
      }
    ...
    // Volatiles
    pokemon.volatiles = setConditions(
      gen,
      "Volatile Status",
      options.volatiles,
    );
    ...
    for (const stat of gen.stats) {
      ...
      if (typeof val === "number") {
        ...
        if (
          typeof options.ivs?.[stat] === "number" &&
          gen.stats.toDV(options.ivs[stat]) !== dv
        ) {
          throw new Error(
            `${stat} DV of '${dv}' does not match IV of '${options.ivs[stat]}'`,
          );
        }
    ...
    setSpc(
      ...
      "ivs",
      options.dvs,
      gen.stats.toIV.bind(gen.stats),
    );

    if (move) {
      move = typeof move === "string" ? move : move.name || "";
      setHiddenPowerIVs(gen, pokemon as { level: number; ivs: StatsTable }, [
        move,
      ]);
    }
    ...
    if (options.boosts) {
      for (const b in options.boosts) {
        ...
        if (typeof val === "number")
          pokemon.boosts[boost] = bounded("boosts", val);
      }
    }
    ...
    if (options.maxhp) {
      if (options.maxhp < pokemon.maxhp) {
        throw new RangeError(
          `maxhp ${options.maxhp} less than calculated max HP ${pokemon.maxhp}`,
        );
      }
    ...
    if (!(pokemon.hp >= 0 && pokemon.hp <= pokemon.maxhp)) {
      throw new RangeError(
        `hp ${pokemon.hp} is not within [0,${pokemon.maxhp}]`,
      );
    }
    if (typeof options.hp === "number" && typeof computed === "number") {
      if (pokemon.hp !== computed) {
        throw new Error(
          `hp mismatch: '${computed}' does not match '${pokemon.hp}'`,
        );
      }
    }
  ...
  static createMove(
      ...
      if (m) {
        ...
        if (m[1]) {
          ...
          if (options.useZ !== undefined && options.useZ !== useZ) {
            throw new Error(
              `useZ mismatch: '${options.useZ}' does not match '${useZ}'`,
            );
          }
          options.useZ = useZ;
        }
        if (m[3]) {
          ...
          if (base?.id === "conversion" && n === 2) {
            ...
            if (options.magnitude && options.magnitude !== n) {
              throw new Error(
                `Magnitude mismatch: '${options.magnitude}' does not match '${n}'`,
              );
            }
            options.magnitude = n;
          } else {
            if (options.basePower && options.basePower !== n) {
              throw new Error(
                `Base Power mismatch: '${options.basePower}' does not match '${n}'`,
              );
            }
            options.basePower = n;
          }
    ...
    if (
      options.name &&
      !(toID(options.name) === base.id || toID(options.name) === toID(name))
    ) {
      throw new Error(
        `Move mismatch: '${options.name}' does not match '${base.name}'`,
      );
    }
    ...
    if (useMax && options.useZ) {
      throw new Error(
        `Cannot use ${base.name} as both a Z-Move and a Max Move simulataneously`,
      );
    }
    if (useMax || move.isMax) {
      if (options.hits && options.hits > 1) {
        throw new Error(
          `'${options.hits}' hits requested but Max Moves cannot be multi-hit`,
        );
      }
    } else if (options.useZ || move.isZ) {
      if (options.hits && options.hits > 1) {
        throw new Error(
          `'${options.hits}' hits requested but Z-Moves cannot be multi-hit`,
        );
      }
    } else {
      if (base.multihit) {
        ...
        throw new Error(
          `'${options.hits}' hits requested but ${base.name} is not multi-hit`,
        );
      }
    }
    ...
    if (typeof options.magnitude === "number") {
      if (move.id !== "magnitude") {
        throw new Error(
          `magnitude ${options.magnitude} incorrectly set on move '${base.name}'`,
        );
      }
    ...
    if (options.consecutive && toID(pokemon.item) !== "metronome") {
      throw new Error(
        "consecutive has no meaning unless the Pokemon is holding a Metronome",
      );
    }
  ...
  static mergeSet(
    ...
    if (set.happiness !== undefined)
      pokemon.happiness = bounded("happiness", set.happiness);
    ...
    if (set.evs)
      setValues(
        ...
        pokemon,
        "evs",
        gen.stats.fill(set.evs, gen.num <= 2 ? 252 : 0),
      );
    ...
    if (
      ...
      while (
        pokemon.evs!.atk! > 0 &&
        2 * 80 + ivs + floor(pokemon.evs!.atk! / 4) + 5 > 255
      ) {
        pokemon.evs!.atk! -= 4;
      }
    }
...
function invalid(gen: Generation, k: string, v: any): never {
  throw new Error(
    `Unsupported or invalid ${k} '${v}' for generation ${gen.num}`,
  );
}
...
export function bounded(key: keyof typeof BOUNDS, val: number, die = true) {
  ...
  if (!ok && die)
    throw new RangeError(
      `${key} ${val} is not within [${BOUNDS[key].join(",")}]`,
    );
  return val;
}
...
function bestMatch(
  ...
  for (const set of sets) {
    ...
    if (!match(set.species, pokemon.species.name)) {
      throw new Error(
        `Received invalid ${set.species} set for ${pokemon.species.name}`,
      );
    }
  ...
  if (!scored.length)
    throw new Error(`Received no sets for ${pokemon.species.name}`);
...
function setAbility(
  gen: Generation,
  pokemon: { species: Specie; ability?: ID },
  name?: string,
) {
  if (name) {
    const ability = gen.abilities.get(name);
...
export function setGender(
  ...
  const gender =
    gen.num === 1 ? undefined : atkDV >= species.genderRatio.F * 16 ? "M" : "F";
  if (name) {
    if (gen.num === 1) throw new Error("Gender does not exist in generation 1");
    if (species.gender && name !== species.gender) {
      throw new Error(
        `${species.name} must be '${species.gender}' in generation ${gen.num}`,
      );
    }
    if (gen.num === 2) {
      if (setAtkDV && name !== gender) {
        throw new Error(
          `A ${species.name} with ${atkDV} Atk DVs must be '${gender}' in gen 2`,
        );
      }
  ...
  if (data) {
    if (Array.isArray(data)) {
      for (const d of data) {
        ...
        if (k !== kind) {
          throw new Error(
            `'${name} is a ${k} not a ${kind} in generation ${gen.num}`,
          );
        }
        obj[toID(name)] = {};
      }
...
function validateStats(gen: Generation, pokemon: State.Pokemon) {
  ...
  for (const stat of gen.stats) {
    ...
    if (actual !== pokemon.stats[stat]) {
      ...
      throw new Error(
        `Expected a ${s} stat of ${actual}, received: ${pokemon.stats[stat]}`,
      );
    }
  }
  return pokemon;
